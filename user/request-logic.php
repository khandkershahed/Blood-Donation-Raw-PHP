...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']) {
    ...
    try {
        ...
        if ($request_id) {
            ...
            // Insert new request
            $query = "INSERT INTO requests (donor_id, requester_id, requester_name, requester_phone, requester_email, blood_type, message, location, urgency) 
                      VALUES (:donor_id, :requester_id, :requester_name, :requester_phone, :requester_email, :blood_type, :message, :location, :urgency)";
            ...
            $stmt->bindValue(':requester_name', $requester_name, PDO::PARAM_STR);
            $stmt->bindValue(':requester_phone', $requester_phone, PDO::PARAM_STR);
            $stmt->bindValue(':requester_email', $requester_email, PDO::PARAM_STR);
            $stmt->bindValue(':blood_type', $blood_type, PDO::PARAM_STR);
            $stmt->bindValue(':message', $message, PDO::PARAM_STR);
            $stmt->bindValue(':location', $location, PDO::PARAM_STR);
            $stmt->bindValue(':urgency', $urgency, PDO::PARAM_STR);
